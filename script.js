...
new Promise(function(resolve) {
  ...
  results.addEventListener('click', e => {
    if(e.target.getAttribute('data-role') === 'rewind' || e.target.getAttribute('data-role') === 'progressbar') {
      let progressBarElem = document.querySelector('[data-role=rewind]'),
          elemWidth = progressBarElem.getBoundingClientRect().right - progressBarElem.getBoundingClientRect().left,
          rewindWidth = e.clientX - progressBarElem.getBoundingClientRect().left,
          progress = parseInt(rewindWidth/elemWidth*100);
      globalPlayer.pause();
      globalPlayer.currentTime = progress/(100/globalPlayer.duration);
      setTimeout( () => globalPlayer.play(), 1);
    }
  });
